...
{                                                                       //OM SANT går programmer in i if-satsen
  ...
  {    
       ...
       (                                                                //Gör att den visas korrekt i webbläsaren
        ' för att du skickat in din anmälan!' .                         //text tylad enligt div css
        '<br>' .                                                        //brake, radbryt med html-kod
        ' En bekräftelse kommer att skickas till din angivna e-postadress' .    //text tylad enligt div css
        '<br>' .                                                        //brake, radbryt med html-kod
        $row["email"] .                                                 //skriver ut senaste angivna svars email, ger konfirmation på angiven email
         '</p>' .                                                       //Avslut paragraf-tagg
         '</div>'                                                       //Avslut div-tagg
          );
        $utfname = utf8_decode($row["name"] );                          //Används för att konvertera tillbaka namnet till utf-8 efter tidigare invertering vid tackmeddelande
        
 			$mailadress = $row["email"];                                //Lagrar denna email i en variabel för mer strukturerad input i mail()
			$id = $row["id"]; 
			$info = 'From: info@stallgard.se' . "\r\n" .    'Reply-To: info@stallgard.se' . "\r\n" .    'X-Mailer: PHP/' . phpversion(); // skapar en string med e-postinfo, användas som headers,skickas med PHP. Specifikt så sätter den en avsändaradress ('From'), en svarsmottagare ('Reply-To') och information om vilken version av PHP som används ('X-Mailer').
			$msg =                                                      //skapar en variabel för email-meddelande
			"Hej " .                                                    //"" används då mailmeddelande i form av string uttrycks med php
			 $utfname .                                                 //insert omkonverterat namn, korrekt uttryckt med utf-8
			 "\r\n" .                                                   //Är ett sätt att radbryta text i php (\n förflyttar ned, \r förflyttar tillbaka markör till början av rad)
			"Tack för att du registrerat dig för vårt nyhetsmail. Vi ser fram emot att träffa dig i sommar!" .  //Meddelande på nya raden
			"\r\n" .                                                    //Samma radbrytning och tillbakaflyttning av markör utförs
			"Hälsningar Edeby Camping"   .                               //Text på sista rad
			"\r\n" .                                                    //Samma radbrytning och tillbakaflyttning av markör utförs
			"\r\n" .                                                    //Samma radbrytning och tillbakaflyttning av markör utförs
			"Klicka på länken nedan för att aktivera kontot." . 
			"\r\n" .                                                    //Samma radbrytning och tillbakaflyttning av markör utförs
			
			"https://stallgard.se/camping2/verify.php?id=" . 
			$id
			;
			mail($mailadress, 'Bekräftelse från Edeby Camping',$msg,$info); //mailfunktionen är en inbyggd funktion som lägger till mottagares email, subject: bekräftelse etc, meddelande, samt info för den som skickat mailet
